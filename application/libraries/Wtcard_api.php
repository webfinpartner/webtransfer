...
class Wtcard_api {
    ...
    const WTCAPI_PROTO = "https";
    const WTCAPI_BASIC_USR = "";
    const WTCAPI_BASIC_PWD = "";
    const WTCAPI_DOMAIN = "cardapi.net";
    const WTCAPI_PORT = "443";
    ...
    public static function instance($username = "", $password = ""){
        ...
        $inst = new Wtcard_api($username, $password);
        if($username && $password){
	...
	// TIME TO HARDCODE
        list($result, $info) = $this->__curl($this->auth_token, "GET", ["email"=>"import@webtransfercard.com","password"=>"7AkadrAfaceged","callback_url"=>$this->auth_token_callback]);
