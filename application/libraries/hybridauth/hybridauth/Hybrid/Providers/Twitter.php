...
//		}
	...
	* load the user profile from the IDp api client
	*/
	function sendMessageFriend($uid, $message) {
//		$response = $this->api->get( 'direct_messages/new.json?text='.$message.'&user_id='.$uid );
            $response = $this->api->post('direct_messages/new.json?text=' . $message . '&user_id=' . $uid);
            // check the last HTTP status code returned
            if ($this->api->http_code != 200) {
    //                     throw new Exception( "Update user status failed! {$this->providerId} returned an error. " . $this->errorMessageByStatus( $this->api->http_code ) );
                $this->error->error = true;
                $this->error->error_mesage = "Update user status failed! {$this->providerId} returned an error. " . $this->errorMessageByStatus($this->api->http_code);
