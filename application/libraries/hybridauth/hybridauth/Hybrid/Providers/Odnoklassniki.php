  ...
  function loginFinish()
	...
	*/
	function getUserProfile()
	{
        ...
        {
            $sig = $this->getSig('friends.get');

            $response = $this->api->api( '?application_key=' . $this->config['keys']['key'] . '&method=friends.get&sig=' .$sig); 
//            var_dump($response);
            if ( ! isset( $response->uid ) ){
//                    throw new Exception( "User profile request failed! {$this->providerId} returned an invalid response.", 6 );
//                    return FALSE;
    ...
    //            var_dump($response);
        ...
        private function getSig($method){
            return md5('application_key=' . $this->config['keys']['key'] . 'method='.$method . md5($this->api->access_token . $this->api->client_secret));
        }
        
        
        
        public function sendMessageFriend($uid, $message){
           $selfUser = $this->getUserProfile();
            
           $params = array(
               'application_key' => $this->config['keys']['key'],
               'method' => 'friends.get',
               'uid' => $selfUser->identifier,
               'uids' => $uid,
               'text' => $message,
               'devices' => 'WEB'
           ); 
           ...
           if ( isset( $response->error_code ) || isset($response->errors) ){
                $this->error->error = true;
        ...
        private function _signServerServer(array $request_params) {
            ...
            return md5($params . md5($this->api->access_token . $this->api->client_secret));
        }
        
        private function _getUrlParams(array $request_params){
            $sig = $this->_signServerServer($request_params);
            
            return '?'.http_build_query($request_params+array('sig'=>$sig));
        }
}
