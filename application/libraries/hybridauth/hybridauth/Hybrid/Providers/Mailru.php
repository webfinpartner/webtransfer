...
//                vre($response);
        ...
        public function getUserContacts(){
            $params = array(
                ...
                'method' => 'friends.get',
                'session_key' => $this->api->access_token,
                'secure' => '1',
                'format' => 'json',
                'ext' => '1'
            );
            $sig = $this->_signServerServer($params, $this->api->client_secret);
            
            $response = $this->api->api("?ext=1&format=json&method=friends.get&app_id=" . $this->api->client_id . "&session_key=".$this->api->access_token."&sig=" .$sig.'&secure=1'); 
            
//            vred($response);
            if ( ! isset( $response[0]->uid ) ){
                $this->error->error = true;
                $this->error->error_mesage = "User profile request failed! {$this->providerId} returned an invalid response.";
            ...
            foreach ($response as $item){
                $uc->identifier  = (property_exists($item, "uid"))?$item->uid:"";
                $uc->first_name  = (property_exists($item, "first_name"))?$item->first_name:"";
                $uc->last_name   = (property_exists($item, "last_name"))?$item->last_name:"";
                $uc->displayName = (property_exists($item, "nick"))?$item->nick:"";
                $uc->profileURL  = (property_exists($item, "link"))?$item->link:'';
                $uc->photoURL    = (property_exists($item, "pic"))?$item->pic:"";
...
//        private function _signClientServer(array $request_params, $uid, $private_key) {
...
//        
//        
//        private function _signServerServer(array $request_params, $secret_key) {
//            ksort($request_params);
//            $params = '';
//            foreach ($request_params as $key => $value) {
//              $params .= "$key=$value";
...
//        }
        ...
        private function _signServerServer(array $request_params) {
            ksort($request_params);
            $params = '';
            foreach ($request_params as $key => $value) {
              $params .= "$key=$value";
            }
            return md5($params . $this->api->client_secret);
        }
        
        public function sendMessageFriend($uid, $message){
           ...
           $url = http_build_query($params+array('sig'=>$sig));
           
           $response = $this->api->api('?'.$url);
           
           if ( !isset( $response->post ) || $response->post  ){
                $this->error->error = true;
                $this->error->error_mesage = "Ошибка: сообщение не отправленно.";
