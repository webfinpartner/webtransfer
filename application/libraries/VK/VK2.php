...
class VK
{
    ...
    public function getAccessToken($code, $callback_url = 'https://api.vk.com/blank.html')
    {
        ...
        $req = $this->createUrl(self::ACCESS_TOKEN_URL, $parameters);
//        echo "<br>$req<br>";
//        $rs  =  file_get_contents( $req );//$this->request( $req );
        $responce  = file_get_contents( $req );
//        $responce  = $this->request( $req );

//        var_dump(  file_get_contents( $req ) );
//        reuturn;

        $rs = json_decode( $responce, true );
//        var_dump($rs);
        if (isset($rs['error'])) {
