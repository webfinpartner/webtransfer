...
<div style="float: right;">
    <!--a id="content_admin_payment_bonus_sb_reject" style="margin: 5px 0px;float:right;" class="button redB" title="" href="#">
        <span>Отменить <?//=$view_all['one']?></span>
    </a>
    <a id="content_admin_payment_bonus_sb_apply" style="margin: 5px 0px;float:right;" class="button greenB" title="" href="#">
        <span>Подтвердить<?//=$view_all['one']?></span>
    </a-->    
...
//                        if( a.diff_acc_credits[ip]) $(this).addClass('diff_acc_credits');
    ...
    function addAction(){
        $(".row_on_click").click(function(){
            ...
            window.location = "/opera/users/"+id;
            return false;
        });
        ...
        $("#content_admin_payment_bonus_sb_apply").click(function(){
            var field = "id";
            var id = $(this).find("["+field+"]").attr(field);
            window.location = "/opera/<?=$controller?>/bonus_sb_apply/"+id;
            return false;
        });        
        
        $("#content_admin_payment_bonus_sb_reject").click(function(){
            var field = "id";
            var id = $(this).find("["+field+"]").attr(field);
            window.location = "/opera/<?=$controller?>/bonus_sb_reject/"+id;
            return false;
        });                
    };
</script>