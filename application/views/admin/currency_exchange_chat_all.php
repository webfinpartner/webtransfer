...
<!--	<a id="content_admin_payment_all_annul" style="margin: 5px 0px;float:right;" class="button redB" title="" href="/opera/<?=$controller?>/annul">
    ...
    <a id="content_admin_payment_all_create" style="margin: 5px 5px;float:right;" class="button greenB" title="" href="/opera/currency_exchange_chat/close_inactive_chats"><span>Закрыть неактивные</span></a>
</div>
<? } ?>
...
//                        if( a.diff_acc_credits[ip]) $(this).addClass('diff_acc_credits');
    ...
    function addAction(){
        $(".row_on_click").click(function(){
            ...
            window.open("/opera/<?=$controller?>/"+id);
            return false;
        });
        
        $(".col_id_user_id").click(function(){
            var field = "user_id";
            var id = $(this).attr(field);
//            console.log(id);
            window.open("/opera/users/"+id, '_blank');
            return false;
        });
