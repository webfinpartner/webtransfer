...
<div id="out_send_window" class="popup_window" style="z-index: 99999; display: block;">
    ...
    <form action="/opera" method="POST">
        <h2><?=_e('accaunt/token_error_2')?> <?=  Permissions::$SEC_TYPES[$admin->sec_auth]?></h2>
        ...
        <button type="submit" name="wt_token" value="<?=_e('accaunt/token_error_4')?>" class="button narrow"><?=_e('accaunt/token_error_5')?></button>
    </form>
		<div style="font-size:12px;float:right;"><a href="/opera/auth/logout"><?=_e('accaunt/token_error_6')?></a></div>
</div>
<script>
    $("#out_send_window").show();
    $(".res-message").html("<?=_e('accaunt/token_error_7')?>").show();
    $("#sms").click(function(){
        $(".res-message").html("").hide();
