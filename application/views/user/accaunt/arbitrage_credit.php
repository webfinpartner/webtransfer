...
//$credit = $max_arbitration_sum;
$default_bonus_account = 6;
$credit = $max_arbtr_6 = $rating_bonus['max_arbitrage_calc_by_bonus'][$default_bonus_account]- $rating_bonus['arbitration_active_summ_by_bonus'][$default_bonus_account];// $rating_bonus['max_arbitrage_calc_by_bonus'][$default_bonus_account];
//$credit = $max_arbtr_6 = 500;
//$rating_bonus['max_arbitrage_days_by_bonus'][6] = 5;

if( $credit < 0 ) 
    $credit = 0;
if( $credit < 50 && $isNewbie ) $credit = 50;
$credit = min(1000,$credit);
...
<div class="widget">
    ...
    <form class="form" id="credit_form" action="<?=site_url('account/my_credits')?>" method="post">
        <fieldset >
			 ...
			 <div class="col">
                        ...
                        <select id="payment_account" name="payment_account" onchange="on_payment_account_change()" class="form_select" style="width: 200px"><? 
                         ...
                         //   echo "<option value='2'$selected>WTUSD&#10084; - $ {$rating_bonus['max_arbitrage_calc_by_bonus'][2]}</option>";      
                           // $max_arbtr = $rating_bonus['max_arbitrage_calc_by_bonus'][6];
                            $selected = ( $default_bonus_account==6 )?' selected="selected" ':'';
                            echo "<option value='6'$selected>WTDEBIT - $ $max_arbtr_6</option>";                                                        
                        ?></select>
                </div>
            ...
            <div class="formRow" style=" height: 140px;">
                <!--div class="col">
                ...
                <div class="col">
                    <label><?=_e('accaunt/arbitration_4')?></label>
                    <select id="select-percent-arbitr" name="percent" class="form_select"><? getPsntSelect($persent) ?></select>
                </div>                                
            ...
            <div style="height: 5px; position: relative; top: 15px;">
                <button class="button" id="arbitration" style="margin-top: 10px;" type="submit" name="submit" ><?=_e('accaunt/arbitration_8')?></button>
...
<?=_e('Кредит на арбитраж временно недоступен')?>
                </div>
        ...
        <div class="popup modal fade in modal-visa-card creds" id="popup_debit_desabled" role="dialog" aria-hidden="false" style="left: 0px; display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="close" onclick="$('.popup').hide('slow');"></div>
                    <h4 class="modal-title"><?=_e('Внимание ')?></h4>
                </div>
                <div class="modal-body" style="padding:20px;text-align:center;">
<?=_e('Арбитраж для DEBIT отключен')?>
                </div>
...
<script>
    var limits = { 2: <?=$rating_bonus['max_arbitrage_calc_by_bonus'][2]?>, 5:<?=$rating_bonus['max_arbitrage_calc_by_bonus'][5]?>, 6:<?=$max_arbtr_6?> };
    var limits_days = { 2: <?=$rating_bonus['max_arbitrage_days_by_bonus'][2]?>, 5:<?=$rating_bonus['max_arbitrage_days_by_bonus'][5]?>, 6:<?=$rating_bonus['max_arbitrage_days_by_bonus'][6]?> }; 
     ...
     function on_payment_account_change(){
                        ...
                        function send_ajax() {
                            ...
                            data.percent = $('#select-percent-arbitr').val();
                    ...
                    $(function() {
                        ...
                        $('#arbitration').click(function() {
                            <? if (!config_item('arbitration_b6_enabled')){ ?>
