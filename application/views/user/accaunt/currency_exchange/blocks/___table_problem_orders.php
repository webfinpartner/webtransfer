        ...
        <tbody class="table_body"> 
          ...
          <?php
            ...
            <tr class="table_row_header htitle">
                <td class="table_cell">
                    <?php echo isset($val->original_order_id)?$val->original_order_id:$val->id; ?> <?php $this->load->view('user/accaunt/currency_exchange/blocks/not_read_message_chat.php', array('order' => $val)); ?>
                        ...
                        <?php if ($user_id ==  $val->seller_user_id  && $val->buyer_confirmed != 1 && !($val->status >= Currency_exchange_model::ORDER_STATUS_HAVE_PROBLEM && $val->status < Currency_exchange_model::ORDER_STATUS_SUCCESS)): ?>
                            ...
                            <?php elseif($val->status == Currency_exchange_model::ORDER_STATUS_PROCESSING || $val->status == Currency_exchange_model::ORDER_STATUS_PROCESSING_SB): ?>
                                ...
                                <form action="" method="post">
                                    ...
                                    <a href="#" class="redB but" onclick="$(this).parent().submit(); return false;"><?=_e('currency_exchange/table_search/delete')?></a>
                                </form>
