...
foreach( $payment_systems_groups as $group )
{
 ...
 ?>
    ...
    <a class="onehrine <?= ($i == 1?'first active_group_tab':'') ?>" data-group-id="<?=$group->id?>" onclick="togle_and_switch_text(event, $(this).next('div.group_id_<?=$group->id?>'), $(this).find('.dop_group_arrow'));/*$(this).next('div.group_id_<?=$group->id?>').toggle('normal', function(){if()});*/ return false;">
         <div class="onehlug"></div>
         <div class="onehldopug"></div>
         <?php if($group->public_path_to_icon): ?>     
             <!--<div class="onehlineico" style="background: url('<?//= $group->public_path_to_icon ?>') no-repeat center center"></div>-->
         ...
         <div class="onehlinetext">                                
             ...
             <?php /* ?>
             <span class="dop_group_arrow">&#9658;</span>  <<?=$group_cont;?>><?php  echo $group->human_name ?></<?=$group_cont;?>>
             <?php */ ?>
...
//                 $name = 'currency_name_'.$payment_system->machine_name;
                 ...
                 //$currency = get_currency( $payment_system->currency_id );
             ?>
    ...
    <?php if(isset($save_user_data) && $save_user_data && $group->show_add_new == 1): ?>
        ...
        <div class="add_new_user_payment_system" style="display:none;">
            ...
            <!--<div style="position: absolute; margin-top: -30px; margin-left: 10px;  padding: 0 3px; height: 20px;">-->
