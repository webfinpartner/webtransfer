...
<html>
    <head></head>
    <body>
        <h2><?=sprintf(_e('currency_exchange/mail/operator_send_message/title'), isset($order->original_order_id)?$order->original_order_id:$order->id)?></h2>
        <br/>
        <h3><?= _e('currency_exchange/mail/operator_send_message/details')?> </h3>
        <table>
            ...
            <?php endif ?>
            <tr><td><?= _e('currency_exchange/mail/operator_send_message/create_date')?></td><td><?=date('d.m.y',strtotime($order->seller_set_up_date)) ?></td></tr>
            <tr>
                <td><?= _e('currency_exchange/mail/operator_send_message/fee')?></td>
                <td>
            ...
            <?php /* if(!empty($buyer_user) && $curent_user_id != $buyer_user->id_user):?>
                <tr><td><?= _e('currency_exchange/mail/operator_send_message/contr')?></td><td><?=$buyer_user->name?> <?=$buyer_user->sername?></td></tr>
            <?php elseif(!empty($buyer_user) && $curent_user_id == $buyer_user->id_user):?>
                <tr><td><?= _e('currency_exchange/mail/operator_send_message/contr')?></td><td><?=$seller_user->name?> <?=$seller_user->sername?></td></tr>
            <?php endif; */?>
        ...
        <br/><br/>
        <a href="<?= site_url('account/currency_exchange/my_sell_list'); ?>#<?=isset($order->original_order_id)?'oid='.$order->id:'id='.$order->id ?>"><?= _e('currency_exchange/mail/operator_send_message/see_order_status')?></a>
        <br/><br/>
        <?= _e('currency_exchange/mail/operator_send_message/signature')?>
    </body>
</html>