...
<html>
    <head></head>
    <body>
        <h2><?=sprintf(_e('currency_exchange/mail/order_need_send_money/title'), isset($order->original_order_id)?$order->original_order_id:$order->id)?></h2>
        <br/>
        <h3><?= _e('currency_exchange/mail/order_need_send_money/details')?> </h3>
        <table>
            <tr>
                <td><?= _e('currency_exchange/mail/order_need_send_money/i_give')?>: </td>
                <td>
            ...
            <tr>
                <td><?= _e('currency_exchange/mail/order_need_send_money/getting')?>: </td>
                <td>
                    <?php foreach ($order->payment_systems as $val): ?>
            ...
            <tr><td><?= _e('currency_exchange/mail/order_need_send_money/create_date')?></td><td><?=date('d.m.y',strtotime($order->seller_set_up_date)) ?></td></tr>
            <tr>
                <td><?= _e('currency_exchange/mail/order_need_send_money/fee')?></td>
                <td>
            ...
            <?php /* if(!empty($buyer_user) && $curent_user_id != $buyer_user->id_user):?>
                <tr><td><?= _e('currency_exchange/mail/order_need_send_money/contr')?></td><td><?=$buyer_user->name?> <?=$buyer_user->sername?></td></tr>
            <?php elseif(!empty($buyer_user) && $curent_user_id == $buyer_user->id_user):?>    
                <tr><td><?= _e('currency_exchange/mail/order_need_send_money/contr')?></td><td><?=$seller_user->name?> <?=$seller_user->sername?></td></tr>
            <?php endif; */?>
        ...
        <a href="<?= site_url('account/currency_exchange/my_sell_list'); ?>#<?=isset($order->original_order_id)?'oid='.$order->id:'id='.$order->id ?>">
            <?= _e('currency_exchange/mail/order_need_send_money/see_order_status')?>
        </a>
        <br/><br/>
        <?php /* ?>
        <a href="<?= site_url('account/currency_exchange/my_sell_list'); ?>?id=<?= $order->id ?>&action=locate">
            <?= _e('currency_exchange/mail/order_need_send_money/confirm_curret_out')?>
        </a>
        <br/><br/>
        <?php */ ?>
       <?= _e('currency_exchange/mail/order_need_send_money/signature')?>
    </body>
</html>