...
<head>
    ...
    <title><?=_e('accaunt/token_error_1')?></title>
<style>
    #buttons{
        display: block;
        margin: 10px 0 20px;
...
<div id="out_send_window" class="popup_window" style="z-index: 99999; display: block;">
    ...
    <form action="<?= site_url('/') ?>" method="POST">
        <h2><?=_e('accaunt/token_error_2')?></h2>
				<div style="font-size:12px;"><?=_e('accaunt/token_error_3')?></div>
        <div id="sms-code" class="formRow padding10-40" style="border-bottom:0;">
            ...
            <div class="res-message" style="color:red">

            </div>

        </div>
        <button style="margin: 0px auto; font-family: 'Arial','Helvetica',sans-serif;" type="submit" name="wt_token" value="<?=_e('accaunt/token_error_4')?>" class="button narrow"><?=_e('accaunt/token_error_5')?></button>
    </form>
		<div style="font-size:12px;float:right;"><a href="<?= site_url('login/out') ?>"><?=_e('accaunt/token_error_6')?></a></div>
</div>
<script>
    $("#out_send_window").show();
    $(".res-message").html("<?=_e('accaunt/token_error_7')?>").show();
</script>
