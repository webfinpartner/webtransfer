$(function() {
	...
	$(".maskPrice").maskMoney( {thousands:' ', decimal:',', precision:0}).trigger('mask');
...
================================================== */
	...
	//===== Sortable columns =====//
	$("table").not("[class^=easyui]").tablesorter(); // not all tables need



	//===== Dynamic data table =====//
	window.sortTh = window.sortThNum = 0;
	$('.dTable thead tr th').each(function (){
		if($(this).text().match(/Дата/)) window.sortTh = window.sortThNum;
		window.sortThNum = window.sortThNum+1;
	});
	oTable = $('.dTable').dataTable({
		"bJQueryUI": true,
		"aaSorting": [[ window.sortTh, "desc" ]],
...
//	});
	$( ".datepicker" ).datetimepicker({
		autoSize: true,
		appendText: '(yyyy-mm-dd HH:mm:ss)',
        timeFormat: 'HH:mm:ss',
		dateFormat: 'yy-mm-dd'
	});
