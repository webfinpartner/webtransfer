...
mn.get_is_created_phone = '/'+mn.site_lang+'/account/ajax_user/is_created_phone';
...
mn.recaptcha = {
    ...
    response: 0,
    widgetId: 0,
    default_id_elem: 'security-recaptcha',
    ...
    //SETTERS
    set_sitekey:function( site_key ){
        this.recaptcha_sitekey = site_key;
        return this;
    },
    
    set_id_elem:function( id_elem ){
        if( !mn.empty( id_elem ) ) this.default_id_elem = id_elem;
        return this;
    },
    
    //MAIN
    //process: init - add_script_line - load - callback - get_response (for sending to server) - clean (responce)
    reload: function(){
        if( !this.recaptcha_sitekey ) return true;
        grecaptcha.reset();
        return this;
    },
    callback: function( response ){
        
        mn.recaptcha.response = response;        
        return this;
    },
    //clean responce
    clean: function(){
        mn.recaptcha.response = 0;
        return this;
    },
    //load g-recaptcha to element
    load: function( id_elem ){
        mn.recaptcha.set_id_elem( id_elem );
        mn.recaptcha.init();
        if( !mn.recaptcha.recaptcha_sitekey || !window['___grecaptcha_cfg'])
            return true;

        mn.recaptcha.widgetId = grecaptcha.render( mn.recaptcha.default_id_elem, {
            'sitekey' : mn.recaptcha.recaptcha_sitekey,  // required
            'callback': mn.recaptcha.callback  // optional
        });
    ...
    add_script_line: function(){        
        if (window['___grecaptcha_cfg']) return true;
...
//            //console.log( result['security_type'] );
    ...
    window:{
        ...
        show_voice:function(){
            mn.get_ajax( '/'+mn.site_lang + '/account/ajax_user/voice_state', {purpose:'change_security_type'},
            function(res){
                //console.log(res,mn.security_module.window.window_handle.find('.panel-footer.choose') );
                if( res['state'] === undefined ) return false;

                if (res['state'] == true)
                    mn.security_module.window.window_handle.find('.panel-footer.choose').show();
            });
        },
        message_timer:{
            handle: null,
        ...
        input:{
            ...
            val: function( val ){
                ...
                if( val !== undefined )
                {
                    mn.security_module.window.input.handle.val( val.replace(/[^-0-9]/gim,'') );
                    return this;
                }
                return mn.security_module.window.input.handle.val().replace(/[^-0-9]/gim,'');
            }
        },
        select:{
            handle : null,
    ...
    loader:{
        ...
        centered : function(html){
            console.log(mn.security_module.loader.loader_handle);
            var lh = mn.security_module.loader.loader_handle;
            var lw = lh.innerWidth();
...
//            console.log( 'loader-init');
    ...
    show_window : function( purpose, security_type ){
        ...
        if(purpose == 'show_set_phone') {
            var data = {};
            data['purpose']  = purpose;

            if( security_type !== undefined ) data['security_type']  = security_type;

             mn.get_ajax( mn.base_url + mn.get_is_created_phone, data,  this.call_back_security_data );
        }
        else {
...
//            mn.security_module.window.handle.remove();
    ...
    call_back_confirm_code : function( result ){
        ...
        if (result['action']) {
            ...
            if (result['action'] == 'change_security_type') {
                mn.security_module.change_security_type(result['data']['purpose'],result['data']['security_type'])
            }
            if ( result['success'] && result['action'] == 'recaptcha.reload') {                
                mn.recaptcha.reload();
            }
        }
        ...
        if( result['success'] )
        {
            ...
            if( mn.security_module.call_back_fn && typeof mn.security_module.call_back_fn !== undefined  )
                if (result['saved_code']) {
                    mn.security_module.call_back_fn({ 'res' : 'success', 'code' : result['saved_code'] });
                } else {
...
//console.log('success-1-');
    ...
    confirm_code : function(){
        ...
        data['recaptcha']  = mn.recaptcha.get_response();
        ...
        //console.log( '-3-' );
		if(this.security_type == 'one_pass') {
            mn.security_module.loader.show(mn._e( 'wait_processing_request' ), 10001);
            mn.security_module.loader.ring.show();
        }
        return false;
    },
    call_back_get_code : function( result )
    {
        ...
        mn.security_module.window.loader.hide(function(){
            ...
            if (result['voice_verification_enabled'] == true) mn.security_module.window.window_handle.find('.panel-footer.choose').show();
    ...
    save_phone:function(){
        ...
        this.backdrop.hide();
        if( mn.security_module.call_back_fn && typeof mn.security_module.call_back_fn !== undefined  )
                mn.security_module.call_back_fn({ 'res' : 'save phone' });
    },
    get_code : function(){
